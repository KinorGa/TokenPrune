...
def extract_answer(
    pred_str, in_the_form=False, separated_by_commas=False, exhaust=False
):
    pred = []
...
def extract_math_answer(question, reasoning):
    ...
    if "separated by commas" in question:
        for ans in extract_answer(
            reasoning, in_the_form=in_the_form, separated_by_commas=True, exhaust=True
        ):
            ans = (
                ans.replace("(", "").replace(")", "").replace("[", "").replace("]", "")
            )
    ...
    # handle cases where desired answer is one (but could be multiple) => return list of strings
        ...
        for ans in extract_answer(
            reasoning, in_the_form=in_the_form, separated_by_commas=False, exhaust=True
        ):
...
def compare_answers(question, parsed_label, parsed_answer):
    ...
    if "separated by commas" in question:
        ...
        return bool(
            set(str(x) for x in parsed_answer) & set(str(x) for x in parsed_label)
        )
...
if __name__ == "__main__":
    ...
    for data in output_data:
        ...
        data["correct"] = compare_answers(
            data["input"], data["parsed_label"], data["parsed_answer"]
        )
